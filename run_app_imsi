        ...
        if not is_key_pressed:
            ...
            for e in events:
                if e.type == pygame.QUIT:
                    running[0] = False
                    exit()
                if e.type == pygame.USEREVENT:  # 키가 눌러졌는지 확인
                    list_move = True
            ...
            if list_move == True:
                move_signal = [random.randint(-1,1) for x in range(10)]
                print(move_signal)
                list_move = False
            if len(move_signal) != 0:
                if move_signal[0] == 1:
        ...
        if is_key_pressed:
            ...
            for e in events:
                if e.type == pygame.QUIT:
                    running[0] = False
                    exit()
                if e.type == pygame.KEYDOWN:  # 키가 눌러졌는지 확인
                    if e.key == pygame.K_UP:  # 박스를 위쪽으로 이동
                        player.movement -= player.speed  # 이동될 위치(좌표만큼)
                    if e.key == pygame.K_DOWN:  # 박스를 아래로
                        player.movement += player.speed  # 특정 좌표만큼
                    if e.key == pygame.K_ESCAPE:
                        ...
                        game_info.pop()
                        return
                if e.type == pygame.KEYUP:  # 방향키를 떼면 멈충
                    if e.key == pygame.K_UP:  # 위 방향키를 똈다면
                        player.movement += player.speed  # 키 눌렸을때 속도만큼 반대로 위치를 더해줌(정지시킴의 목적)
                    if e.key == pygame.K_DOWN:  # 아래 방향키를 똈다면
                        player.movement -= player.speed  # 키 눌렸을때 속도만큼 반대로 위치를 더해줌(정지시킴의 목적)
                if e.type == event_moveup:  # userevent 중 위 시그널일때
                    player.movement = -player.speed  # 특정 좌표만큼 위치 이동
                if e.type == event_movedown:  # userevent 중 아래 시그널일때
                    player.movement = player.speed  # 특정 좌표로 박스 이동
