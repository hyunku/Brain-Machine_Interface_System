...
def play_function() -> None:
    ...
    while True:
        ...
        if ai_ing == False and remainAI_Time <= 0.0 and len(ex_array) > 0:
            ai_direction = ex_array.pop(0)
            if ai_direction == -1:
            ...
            ai_ing = True
            remainAI_Time = AI_DurationTime
            print("direction change")
        elif ai_ing == True and remainAI_Time > 0:
            remainAI_Time -= elapsedTime
        else:
            if ai_ing == True:
                player.movement = 0
